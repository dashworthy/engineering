...
change, what it does, why, and how it was verified. Do not add self-promotion for Claude, Claude
Code, or any other AI/LLM vendor, and do not append AI-authorship or "generated by" attribution:
no `Co-Authored-By: Claude` trailer, no "🤖 Generated with…" footer, no tool advertising anywhere
in the title or body. The PR speaks for the work, not for the tool that helped write it. The
exception is a project that has explicitly asked for such attribution — its stated convention
wins; absent that, leave it out.
