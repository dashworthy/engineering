...
- A returned or rendered string — `return "Order saved"`, `<button>Submit</button>`, a toast/alert
  ...
  localizes the same kind of text through a helper or catalog. The person sees a string that will
  never change with their locale. Name the string and the translation mechanism the surrounding code
...
- **Concatenation / sentence-building from fragments** — `t("You have") + " " + count + " " + t("items")`
  or `t("Welcome") + name`. A translator gets disconnected fragments and cannot reorder them for
  languages whose grammar puts the pieces in a different order. Use a single parameterized message
...
- **Missing plural handling** — a count rendered through one fixed string (`count + " item(s)"`, or a
  ...
  (`_n()`, ICU `{count, plural, …}`, `trans_choice()`). Languages have more than two plural
  categories; the fixed string cannot express them.
...
- A currency built by string (`"$" + amount`, `"€" . $price`) rather than a locale/currency-aware
  formatter — the symbol, placement, decimal/grouping separators, and rounding all vary by locale.
- A date/time with a fixed pattern (`date("m/d/Y")`, `strftime("%m/%d/%Y")`) — `m/d/Y` is not how
  most of the world writes a date; use the platform's locale-aware date formatter.
