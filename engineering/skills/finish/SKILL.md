---
name: finish
description: "The finish phase: when work is complete and green, open the pull request the plan authorized (or, for a stacked run, open the stacked PRs) — the pipeline integrates only by pull request and never merges, directly or by landing a stack; with no plan behind the branch, present the pull-request / cleanup options and ask."
---
...
carried out is judgment applied to whatever this project's own remote, review process, and branch
model happen to require. The pipeline integrates only by opening a pull request; it never merges a
branch to its target — directly or by landing a stack — so no merge option is offered here, ever,
not even as a prompt for the user to agree to.
...
the `code-review` skill's job, not this one.) The `documenting` phase runs immediately upstream of
this skill, so any documentation it wrote or updated rides the branch tip as its own commit and is
part of the whole-branch change reviewed here.
...
the single "Open a pull request" with opening (or refreshing) the stacked PRs; the pipeline
opens the stack and stops — landing it is a human's job outside the pipeline. Which of these are
actually live options depends on the project, not on this skill's own preference — read the remote
...
Whenever this skill writes a pull-request body — opening one, or opening each PR in a stack —
load `references/pr-description.md` and follow it to compose the body rather than drafting
it inline. A commit message this skill writes directly follows the same spirit: about the
...
- **Open a pull request** — for a non-stacked branch, the one way the branch re-enters the
  repository; check whether a PR already exists for
  this branch before offering to open a second one. Open it, hand back its link, and stop
  there — this skill does not chase the PR through review or merge it once it exists; the branch
  is landed by a human, outside the pipeline, once the PR is approved.
- **Open the stacked PRs** — for a stacked run, in place of "Open a pull request": ensure each
  task's pull request in the stack is open and current against its parent (via
  `engineering:using-stacked-pull-requests`), hand back the stack's links, and stop. The pipeline
  never merges the stack — a human lands it bottom-up, outside the pipeline, once the PRs are
  approved.
- **Clean up only** — the branch turned out unneeded, its content already landed another way, or
  ...
  Discarding is destructive in a way opening a pull request is not: when the plan's finish strategy
  authorized the cleanup, that authorization is the
