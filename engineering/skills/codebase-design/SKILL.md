...
is how `review-plans` calls this skill over a plan's Interfaces blocks). In review mode:
...
Review mode judges a shape; it does not own the fix. The caller — `review-plans` — decides which
