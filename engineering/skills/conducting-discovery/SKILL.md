...
1. **Never read an artifact produced by a dispatched subagent.** Not `brief.md` §7, not §8. You route paths and act on RETURN blocks. Files **written in the main thread are yours to read** — `00-request.md` and `open-threads.md`, which stage 1 writes in your own session; that is not an exception, since the prohibition is only on artifacts a *dispatched subagent* authored. `brief.md` is the one file that stays closed despite being partly yours: you wrote §1–§6 in the main thread, but the file — not the section — is the unit, and stage 2's §7–§8 live inside that same file, so the whole of it stays closed to you. Your context stays small and decision-grade — the entire point of the architecture.
2. **Stage 1 is yours.** It is interactive, so it runs in the main thread and you write `brief.md` §1–§6 yourself. That is the one authoring exception — not a licence to read §7 and §8 back once stage 2 has appended them. Handing content *down* to a subagent — as when you dispatch `expanding-scope` with the requirements inline — is likewise not a violation: the rule governs what you read *back*.
...
You will be holding a path to a document you did not write, about a request you have discussed for twenty minutes. The pull to open it is real. Name it, and refuse:

- *"Just to check it came out right"* — nothing checks it, by design. The brief is released to the user unreviewed, deliberately. Opening it restores no check; it only costs you the context discipline. The user is the reader.
- *"The user asked me a question about the brief"* — report the path. If they want it discussed, they can open it or start a fresh conversation with it as input. You answering from a brief you read is the failure mode, not the service.
...
`actionable` carries decision-grade content only: expansion candidates awaiting adjudication, `sections_1_6_lines: <n>` from stage 2, or a `BLOCKED` reason — that is the whole list. It never carries brief prose or reasoning; if there is nothing to act on, it is empty.

`artifact` carries a path, not content. One skill — `engineering:expanding-scope` — writes no file at all, so its `artifact` field carries no path and its `actionable` is its whole output. That is expected, not malformed.
...
| `brief.md` exists | **Ask** the user: re-run **stage 2** against it, or start over at **stage 1**? Starting over at stage 1 rewrites the file from line 1 and discards any §7–§8 a previous run left, because that body was ordered from requirements about to be replaced. Starting over is still warm: `open-threads.md` survives a stage 1 restart, so the coverage table and open threads carry across. Stage 2 then appends a fresh one. A third answer is possible: if the user says the brief is already complete (§7–§8 present), the resume path is simply the release hand-off — hand it to `engineering:brainstorming` without re-running either stage. Ask which they mean; do not open the file to tell. |

Read `00-request.md` freely — you wrote it. The prohibition is on artifacts a subagent authored.

The one question exists because `brief.md` looks the same to you whether stage 1 has just written §1–§6 or stage 2 has already appended §7 and §8 — and **you may not open it to tell those apart.** Do not infer, do not introduce a status marker, and do not carry state between runs; one question costs a single turn. If the user is unsure, stage 1 is the cheaper mistake: re-running stage 2 over a brief with no §1–§6 produces nothing, while re-running stage 1 costs only time. **Do not open `brief.md` to answer the question for them,** and do not have a subagent read it and report back — that is the same read with an extra hop.
...
Obtain the run directory via `run-context.sh` and write `00-request.md` yourself, with the request verbatim, **before anything else** — before triage, before the first question. If the user gave no request text, ask for it first; there is nothing to record otherwise. This directory is what you supply to `engineering:interrogating-requirements`, which writes `brief.md` §1–§6 into it. Every path you hand to a dispatched subagent is inside this directory.
...
**Escape valve.** If the request is genuinely trivial (one-liner, rename, config tweak), say so in one sentence and exit the pipeline. No brief. If in doubt, it is not trivial — interrogate. Before you exit, **append a line to `00-request.md` recording the trivial exit and your one-sentence reason.** Otherwise the run directory is a request with no brief — byte-identical to a run someone abandoned mid-interrogation — and a later re-run under the same slug would re-interrogate a request you already judged trivial. `00-request.md` is your own file, so writing to it and reading it back breaks no rule.
...
It appends **§7 and §8 only**: the dependency-ordered body and the handoff pointer. It does not rewrite, re-word or reorder §1–§6, and neither do you. The finished brief reads in two voices — yours for the requirements, the subagent's for the body — the intended shape, not a defect to smooth over.
...
- **You have no number to compare** — a **resumed run** where the user chose to re-run stage 2 against a `brief.md` from an earlier session. Stage 1 did not run, so no baseline exists, and you may not open the file to reconstruct one. **The check is unavailable, not passed.** Say so explicitly at release: the brief is handed over without the two-writer check, because the run that wrote §1–§6 is over. Do not treat stage 2's reported number as self-verifying — a number with nothing to compare against proves nothing. If the user wants the check, starting over at stage 1 provides it.

Never silently skip this. An unavailable check that goes unmentioned reads exactly like a check that passed.

This is a tripwire, not a proof: it catches inserted or deleted lines but misses a re-wording that preserves the count — which is why the rule in `engineering:sequencing-requirements` is an absolute prohibition, not left to this check. Counting a number you already hold is not reading the artifact.

If it returns `status: BLOCKED` — contradictory requirements, a requirement section that is actually empty, or `brief.md` missing, unreadable, or missing §1–§6 — **§7 and §8 were not written.** Halt and escalate to the user with the reason from `actionable`. Do not retry silently, do not author §7 or §8 yourself, and **do not release the brief.** A `BLOCKED` is usually stage 2 telling you something is wrong with *your* sections — it is forbidden from fixing §1–§6, so reporting is all it can do. Escalate with that reason and **stop there.** If the user decides to fix it, the run re-enters at stage 1; you do not make that decision for them and you do not roll straight into a rewrite. Never release a brief that stops at §6: there is no body, and §8 — the only section telling a reader what the document is for — is missing.
...
2. **Hand off to `engineering:brainstorming` with the path to `brief.md`.** This begins the chain the Overview describes (brainstorming → to-spec → spec gate); signal writes no spec and does not design. Brainstorming is interactive, so it runs in the **main thread** — a hand-off of control to the next skill, not a dispatched subagent you read a RETURN block from. You pass the path, exactly as you handed it to stage 2; you do not read `brief.md` to hand off or afterward.
...
5. State plainly that signal's job is done: the brief is the input to `engineering:brainstorming`, which owns what happens next (see Overview). Make no claim about the approach yourself.
6. Stop.

**Do not read the brief and do not summarise it.** You have never seen §7 or §8 and you do not open them now to describe what you are handing over. The path is the handover — handing the brief to `brainstorming` is not an exception; you hand it over unread, the same way you hand it to stage 2.

**Do not design it, and do not offer to implement it.** Do not sketch an approach, propose a first step, estimate effort, or say "want me to start on §7?". Weighing the approach is `brainstorming`'s job, exactly where the hand-off sends the brief. Building is further still. Signal makes no claim about the approach or the implementation, and neither do you.
...
| User abandons mid-run | Artifacts remain on disk. **Stage 1 writes `brief.md` §1–§6 as soon as the advancement gate is met**, so an interrogation that got that far survives as a real brief whose §5 says scope is unsettled. Abandoned before the gate, `open-threads.md` survives with whatever coverage and threads stage 1 banked, so the next run resumes warm rather than cold. Only a run abandoned before the first answer leaves nothing but `00-request.md`. Say which of the three happened rather than letting the user guess. |
...
**On the no-subagent case:** running inline is a degradation you announce, not a silent fallback. Tell the user context purity is degraded because the work is running in your thread. Never let a missing subagent capability skip a stage — a skipped stage is worse than a dirty context. Stages 1 and 2 produce the same artifacts inline; only your context suffers.
