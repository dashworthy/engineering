#!/bin/sh
# Resolve (creating if needed) the scratch dir for one phase of the current verity run.
#
# Usage: run-context.sh <name> [slug]
#   <name>  phase subdir, e.g. test-hardening | auditing-test-gaps
#   [slug]  short kebab name for a NEW run; ignored if a run is already active.
#
# The active run id lives in .verity/.current-run as "<YYYY-MM-DD>-<slug>".
# The first caller in a session creates it; later callers join it. Prints the
# absolute path of .verity/<run>/<name>/ and ensures it exists.
set -e
