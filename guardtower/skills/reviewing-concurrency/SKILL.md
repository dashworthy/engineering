---
name: reviewing-concurrency
description: "Guardtower's concurrency facet: review a change for race conditions and unsafe interleaving — a check-then-act (TOCTOU) window, a non-atomic read-modify-write, a lost update on a shared counter or balance, shared mutable state touched without synchronization, a compound operation with no enclosing lock or transaction — returning capped, floored, self-contained findings. Use when a concurrency, race-condition, or thread-safety review of a diff/branch/PR is requested."
---
...
4. **Write the artifact and return.** Write the kept findings to `artifact_path` (the facet's
   ...
   suggestion) — each `claim`/`why` legible to a reviewer with no shared context. Name the two
   executions and the state they corrupt, so the interleaving is legible without rerunning the reasoning.
   Write the artifact even when nothing survives the floor (record "no findings above the floor"). Return
   the contract result.
