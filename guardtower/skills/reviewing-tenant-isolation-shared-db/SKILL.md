---
name: reviewing-tenant-isolation-shared-db
description: "Guardtower's shared-DB tenant-isolation facet: review a change in a single-database / shared-schema multi-tenant app for cross-tenant data leaks — a query missing its tenant scope, a global-scope bypass or raw query, a cross-tenant reference by ID, mass-assignment of the tenant discriminator, an unscoped cross-tenant aggregate, an un-namespaced cache key — returning capped, floored, self-contained findings. Use when a tenant-isolation review of a diff/branch/PR in a shared-database multi-tenant application is requested."
---
