---
name: reviewing-api-consumption
description: "Guardtower's API-consumption facet: review a change for how it consumes a remote/HTTP API — over-fetching a payload it does not read (all fields, or an unbounded/unpaginated collection), doing client-side what the API offers server-side (filter/sort/search), excessive call volume (redundant calls, no caching/dedup, request waterfalls, a call per item, over-aggressive polling), and 429 rate-limit safety (no retry/backoff, ignoring Retry-After, request storms) — returning capped, floored, self-contained findings. Use when an API-consumption or fetch-efficiency review of a diff/branch/PR is requested."
---
...
This facet self-limits at the source (see `../reviewing/references/hard-stops.md`), under the
shared `../reviewing/references/facet-contract.md`.
