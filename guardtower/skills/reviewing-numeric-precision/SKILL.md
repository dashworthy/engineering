---
name: reviewing-numeric-precision
description: "Guardtower's numeric-precision facet: review a change for precision and unit defects in arithmetic on meaningful quantities — binary floating point used for money or an exact value, silent rounding or truncation, a unit mismatch (cents vs dollars, ms vs seconds, bytes vs KB), integer overflow, precision lost on a numeric cast, arithmetic mixing currencies or scales without normalizing — returning capped, floored, self-contained findings. Use when a numeric-precision, money-handling, or units review of a diff/branch/PR is requested."
---
