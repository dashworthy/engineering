...
description: "Guardtower's technical facet: review a change for reinventing what a library or an already-imported module already provides (reuse over reinvention), for inefficient data access (N+1, unbounded queries), and for correctness-scoped best-practice defects, returning capped, floored, self-contained findings. Use when a technical review of a diff/branch/PR is requested, or when guardtower's reviewing orchestrator dispatches the technical facet."
---
...
`artifact_path` (`.guardtower/<run>/reviewing-technical/findings.md`), and `caps` (`top_n`, `floor`).
...
- It does not **scan the repository** — its reach is the diff plus already-imported modules (ADR-0004);
