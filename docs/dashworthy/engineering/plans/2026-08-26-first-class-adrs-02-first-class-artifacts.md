...
**Steps:**
- [x] Invoke `skillsmith:testing-skills` against `recording-adrs`, `using-adrs`, and the intake-clause edits: confirm each is discovered when relevant and followed under time/authority pressure, and close any rationalization that lets an agent skip recording a real decision or citing a governing ADR. Record adjustments as ordinary skill edits (re-running the verification gate after each). _(`skillsmith` is not installed; used the equivalent methodology `superpowers:writing-skills/testing-skills-with-subagents.md` — RED-GREEN-REFACTOR pressure testing with subagents. Ran three probes: (1) discovery routing — using-adrs correctly reached before design/build, recording-adrs at the live-alternatives fork, both correctly declined for a typo fix (no over-reach); (2) recording-adrs under time+authority+sunk-cost pressure — chose to record the real fork (Proposed) AND to NOT flood on a naming coin-flip, defeating the lead-authority, "no time", and "later TODO" rationalizations; (3) using-adrs under "I know it cold" pressure — consulted the index, followed the Accepted ADR, skipped the Superseded one, cited by path, and recorded a fresh decision with no approval gate (correctly rejected the imported-convention-gate rationalization). All GREEN on first pass; no new rationalizations survived, so no skill edits were needed and the gate is unchanged/green.)_
...
- [x] Invoke `engineering:conducting-test-hardening` over this plan's whole diff. Confirm the new `validate.sh` / `acceptance.sh` assertions genuinely fail if the doctrine, a new skill, the ADR index, an intake clause, or the passive-idiom guard were reverted — i.e., the tests would catch the regression. This closing task is mandatory and is this plan's last step. _(For a markdown-skill plugin the suite is the shell structural-assertion set, so hardening = mutation-verifying each guarded invariant: revert it, confirm the guarding check goes RED naming the exact failure, restore, confirm green. All 5 confirmed — (1) doctrine `intake trigger` literal → `FAIL - doctrine names the intake-trigger pattern element`; (2) recording-adrs frontmatter `name` → `FAIL - recording-adrs frontmatter names itself`; (3) ADR index 0001 row → `FAIL - ADR index links ADR 0001`; (4) brainstorming `live alternatives` → `FAIL - brainstorming carries the ADR intake clause`; (5) reintroduced a passive read in tdd → guard names the file + `FAIL - no SKILL.md/command/reference reads … a passive 'when present' idiom`. Each restored to green; final full gate `suite.sh` + `acceptance.sh` + `plan03.sh` all exit 0, working tree clean. Hardening exit: **pass** — no unaddressed gap, no breakage.)_
