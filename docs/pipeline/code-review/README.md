...
The unit of the review is a **facet** — one review lens, such as security, concurrency, or accessibility. Rather than run every lens on every change, code review offers a **menu** of facets and lets the human pick. The menu arrives **pre-filled**: a set of core lenses is always checked, and others are pre-checked when the change's character matches them, so the human is confirming a sensible default rather than choosing from a blank slate. They uncheck what they don't want and add what they do.
...
| Facets | `skills/code-review/references/facets/<facet>/facet.md` | One review lens each (19 today). A facet's own doc is read only when it is dispatched — never merely to decide whether to run it. |
...
| `reviewing-data-presentation` | Identity-ambiguous presentation: distinct records a person can't tell apart | Opt-in: alters how records are labeled or identified to a person |
| `reviewing-accessibility` | Perceivability & operability: alt text, labels, ARIA, keyboard/focus, contrast, motion | Opt-in: alters user-facing rendered output |
| `reviewing-i18n` | Untranslated/untranslatable user-facing text; locale-blind formatting | Opt-in: introduces or alters text shown to a person where the project localizes it |
