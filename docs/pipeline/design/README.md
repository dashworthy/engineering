...
| Spec writer & gate | `skills/spec/SKILL.md` | The single writer of the Tier-1 spec and holder of the spec gate. Serializes the recommended design into one spec, writes it as a draft, holds for the human's `Approve`/`Request changes`, and on approval mints the marker and promotes — and publishes a frozen copy to the committed [spec ledger](../spec-ledger/README.md). Does not design, plan, interrogate, or invent. |
