...
- **`receiving-code-review`** is the review-feedback door. A set of review comments enters here, and
  ...
  set — but the per-comment verify and impact-check follow the same inline-vs-fan-out floor
  `code-review` uses: a small reception is checked inline, while a larger one fans each comment (or
  a tight cluster) out to a subagent that shares only a read of the review branch, then reconciles
  the verdicts. Reply and resolve stay on the main thread.
...
| Review-feedback entrance | `skills/receiving-code-review/SKILL.md` | Checks out the original review branch, aggregates the comments, verifies each against the codebase (inline for a small reception, otherwise fanned out one subagent per comment/cluster above the same floor `code-review` uses, then reconciled), impact-checks beyond the commented line, and carries two standing instructions (reply per thread; stack each fix's PR onto the review branch) into the shaped context. |
