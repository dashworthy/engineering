...
| Shared interrogation | `skills/interrogating-requirements/SKILL.md` | The relentless requirement extractor `signal` drives as its primary beat and `triage`, `receiving-code-review`, and `simplify` drive on demand as a fallback. Invoked by name as `engineering:interrogating-requirements`. Interactive, main-thread only; writes `brief.md` §1–§6 and `open-threads.md`. Cannot run as a dispatched subagent. |
...
- **The four converge on design, never on each other.** No entrance invokes another entrance.
  ...
  `engineering:interrogating-requirements` itself as its own discovery leg.
...
discovery leg runs, edit `skills/interrogating-requirements/SKILL.md` (shared — a change there affects
