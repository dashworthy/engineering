...
roughly one file and a handful of comments — is cheaper to work inline than to spin up subagents
for; do it inline. Above that floor, **fan out**: dispatch one subagent per comment, or per tight
...
Then **reconcile** the verdicts in the main context before designing. This mirrors how `code-review`
fans its facets out above the same kind of floor. Aggregation stays inline — the set must be read
whole — and so do reply and resolve below, which touch threads and are partly the user's call.
