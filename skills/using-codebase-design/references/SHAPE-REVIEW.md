...
| Two mechanisms, one concern | A shape implements more than one branch for the same kind of work, and they aren't built the same way — one branch composes through an interface, a sibling branch reimplements the equivalent behavior on its own instead of using it. The codebase now has two ways to do the one thing. | Route every branch through the same seam — **Strategy** for the varying cases, with the divergent branch pulled behind the interface its sibling already uses. |
| Single-use wrapper / needless indirection | The decomposition proposes a named method (or class) that exactly one call site will ever reach, and the name restates the single call it forwards — a `sprintf` and a normalize, a `hash(serialize(...))` — instead of a concept the caller reasons about. The reader must open the body to learn it hides no decision. This is the inverse of the rows above: not too little structure, but a boundary drawn where none was earned. | Inline it — the reverse of `DEEPENING.md` #3: a seam reached once that decides nothing is a name with extra steps, not a module. Keep the seam only when it earns its keep — the name hides genuine complexity, gathers an invariant the call site shouldn't restate, or the call recurs at more than one site. Clarity or reuse earns a seam; relabeling one call does not. |
