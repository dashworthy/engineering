...
// (`.engineering/<run>/using-doc-creation/`), outside this package — but `node --import tsx` only applies the package's
...
// the doc's bare `@engineering/using-doc-creation` import resolves by package self-reference (the `exports` field)
// and react-pdf lays out correctly. The staged copy is removed after each render. The authored file
// never moves and never learns where the package lives — it names the stable `@engineering/using-doc-creation`
