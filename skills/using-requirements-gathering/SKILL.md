---
name: using-requirements-gathering
description: "Interrogate a vague request into hard, unambiguous requirements before any design — refuse vagueness, offer choices and mine the correction, fill every coverage dimension, and write brief.md §1–§6. Interactive and main-thread only. Driven by signal and invoked by triage, receiving-code-review, and simplify for generic requirement mining."
---

# using-requirements-gathering

Say this first, plainly: `Using the using-requirements-gathering skill to interrogate the request into hard requirements.`

> Interactive and main-thread only — this skill cannot run as a dispatched subagent. Invoke it when a
> request must be interrogated into hard requirements.
...
line reads `the suite maintainer` or `@handle`, never a personal address in parentheses.
...
**Stop at §6.** The brief ends at §6 — there is no §7 or §8. Do not sketch further sections, leave placeholder headings, or order the work by dependency; that is design work downstream, not the brief's.
...
Then hand control back to the entrance that invoked you, which routes from there. You do not design, write a spec, plan, or hand off to design yourself; the brief is the deliverable and your job ends at it.
