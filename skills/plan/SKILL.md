...
`engineering:using-diagrams`. A plan that pictures a tricky flow once is easier to build against
than one that leaves every builder to reconstruct it.
...
Before calling the plan finished, run a self-review pass over what was just written: work
`references/self-review-checklist.md` over the plan just written — its five checks (spec coverage,
no deferred request, placeholder scan, type consistency, stacked-PR structure) each name a way a
document-level defect hides, and where one fires, fix it in place rather than noting the gap.
...
capabilities, flags tasks resting on unproven assumptions, and flags over-engineering — unearned
abstraction or speculative scope the spec never asked for — surfacing each to the human as
...
inside it (the reinvention, unproven-assumption, and over-engineering flags). The human gate is still the plan gate below,
